...
<!--      </div>-->
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400" v-if="data.labels != null">
        ...
        <tr>
          ...
          <th v-for="label in data.labels" scope="col" class="px-2 py-3">
            {{ label }}
          </th>
        </tr>
        </thead>
        ...
        <tr v-for="value in data.values" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
          ...
          <td v-for="(v, key) in value" class="px-2 py-4">
            <div v-if="key === 'status'">
            ...
            <div v-else-if="key === 'content'" v-html="v.substring(0, 100) + '...'"></div>
            ...
            <div v-else-if="key === 'parent_id'">
                <span v-if="v === 0" title="Sans catégorie" class="bg-red-100 text-red-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300"><font-awesome-icon :icon="['fas', 'circle-xmark']" /></span>
                <p v-else>{{ v }}</p>
            </div>
            ...
            <div v-else-if="key === 'guide'">
              <RouterLink :to="'/guides/' + v.slug">
                <span class="bg-blue-100 text-blue-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{{ v.title }}</span>
              </RouterLink>
            </div>
